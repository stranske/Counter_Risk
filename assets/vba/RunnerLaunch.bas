...
Public Function BuildCommand( _
    ...
    resolvedOutputDir = ResolveOutputDir(repoRoot, selectedDate)
...
Private Sub RunModeClick(ByVal runMode As String)
    ...
    End If
    WriteResult "Success"
    WriteDataQualityStatus outputDir
    Exit Sub
...
Private Sub WriteDataQualityStatus(ByVal outputDir As String)
    Dim summaryPath As String
    Dim statusColor As String
    summaryPath = outputDir & "\DATA_QUALITY_SUMMARY.txt"
    If Not FileExists(summaryPath) Then
        Exit Sub
    End If
    statusColor = ReadOverallStatusColor(summaryPath)
    WriteDataQualityStatusCell statusColor
End Sub

Public Function ReadOverallStatusColor(ByVal summaryPath As String) As String
    Dim fileNumber As Integer
    Dim lineText As String
    Dim upperLine As String

    ReadOverallStatusColor = ""
    If Not FileExists(summaryPath) Then
        Exit Function
    End If

    fileNumber = FreeFile
    Open summaryPath For Input As #fileNumber
    Do While Not EOF(fileNumber)
        Line Input #fileNumber, lineText
        upperLine = UCase$(lineText)
        If InStr(1, upperLine, "(GREEN)", vbBinaryCompare) > 0 Then
            ReadOverallStatusColor = "GREEN"
            Close #fileNumber
            Exit Function
        End If
        If InStr(1, upperLine, "(RED)", vbBinaryCompare) > 0 Then
            ReadOverallStatusColor = "RED"
            Close #fileNumber
            Exit Function
        End If
        If InStr(1, upperLine, "(YELLOW)", vbBinaryCompare) > 0 Then
            ReadOverallStatusColor = "YELLOW"
            Close #fileNumber
            Exit Function
        End If
    Loop
...
Private Sub WriteDataQualityStatusCell(ByVal statusColor As String)
    Dim cell As Range
    Set cell = ThisWorkbook.Worksheets(RUNNER_SHEET_NAME).Range(DQ_STATUS_CELL)

    Select Case UCase$(Trim$(statusColor))
        Case "GREEN"
            cell.Value = "GREEN - Safe to send"
            cell.Interior.Color = RGB(0, 176, 80)
        Case "YELLOW"
            cell.Value = "YELLOW - Review warnings"
            cell.Interior.Color = RGB(255, 192, 0)
        Case "RED"
            cell.Value = "RED - Do not send"
