...
def test_mapping_diff_report_extracts_unmapped_and_fallback_names_from_manifest_json(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured_call: dict[str, object] = {}
                ...
                "reconciliation_results": {
                    "by_variant": {
                        "all_programs": {
                            "by_sheet": {
                                "Total": {
                                    "counterparties_in_data": ["Known House"],
                                    "mapping_diff_unmapped_source_names": ["  Unknown House  "],
                                    "mapping_diff_fallback_mapped": {"Societe Generale": "Soc Gen"},
                                }
                            }
                        }
                    }
    ...
    assert exit_code == 0
    assert captured_call["input_sources"] == {
        "normalization": ["  Unknown House  ", "Known House", "Societe Generale"],
        "reconciliation": ["  Unknown House  ", "Known House", "Societe Generale"],
    }
