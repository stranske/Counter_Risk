...
def _write_invalid_required_version_workflow(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        """
name: Release
...
def test_verify_release_workflow_dispatch_fails_when_version_input_is_required(
    tmp_path: Path,
) -> None:
    workflow_path = tmp_path / ".github" / "workflows" / "release.yml"
    _write_invalid_required_version_workflow(workflow_path)
