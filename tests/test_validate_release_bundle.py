...
def _write_release_artifacts(bundle_dir: Path) -> None:
    ...
    # Create both names so platform-specific detection does not make tests flaky.
    (bundle_dir / "bin" / "counter-risk").write_text("binary\n", encoding="utf-8")
    (bundle_dir / "bin" / "counter-risk.exe").write_text("binary\n", encoding="utf-8")


def _make_test_env(tmp_path: Path) -> dict[str, str]:
    env = os.environ.copy()
