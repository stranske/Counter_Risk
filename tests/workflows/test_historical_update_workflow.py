...
def test_main_rejects_invalid_date_before_any_workbook_io(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    exposure_summary_path = tmp_path / "exposure-summary.xlsx"
    exposure_summary_path.write_text("placeholder", encoding="utf-8")
    ...
    def _should_not_be_called(*args: object, **kwargs: object) -> object:
        ...
        raise AssertionError("workbook I/O should not run for invalid date input")

    monkeypatch.setattr(historical_update, "locate_ex_llc_3_year_workbook", _should_not_be_called)
    monkeypatch.setattr(historical_update, "calculate_wal", _should_not_be_called)
    monkeypatch.setattr(historical_update, "append_wal_row", _should_not_be_called)

    with pytest.raises(ValueError, match="px_date must be an ISO date string"):
        historical_update.main(
            [
                "--date",
                "01-31-2026",
                "--exposure-summary-path",
