...
**Progress:** 20/20 tasks complete, 0 remaining
...
- [x] Update repository to store VBA modules as source files in `assets/vba/*.bas` (if not already present).
  - [x] Create the assets/vba directory structure if it does not already exist (verify: confirm completion in repo)
  - [x] Extract VBA modules from the Excel workbook (verify: confirm completion in repo) save them as .bas files (verify: confirm completion in repo)
  - [x] Define scope for: Verify that all VBA modules are present in assets/vba with correct file extensions
  - [x] Implement focused slice for: Verify that all VBA modules are present in assets/vba with correct file extensions
  - [x] Validate focused slice for: Verify that all VBA modules are present in assets/vba with correct file extensions
- [x] Write `docs/macro_spec.md` describing each macro’s intent in plain language.
- [x] Add per-macro specification entries to `docs/macro_spec.md` listing required inputs (sheet names, columns).
- [x] Add per-macro specification entries to `docs/macro_spec.md` listing output expectations (ranges affected, invariants).
- [x] Create pytest spec tests using fixtures that perform range-level checks in generated MOSERS-format outputs.
  - [x] Create pytest fixtures that load sample input data for macro testing (verify: confirm completion in repo)
  - [x] Implement pytest tests that verify range-level outputs match expected MOSERS format
  - [x] Add pytest tests that validate cell-by-cell transformations for each macro (verify: tests pass)
- [x] Add pytest spec tests that assert invariants like “no blank numeric cells” and “headers match expected”.
- [x] Update `docs/macro_spec.md` with a “known-acceptable drift” section documenting tolerances and rounding rules.

### Acceptance Criteria
The PR is complete when ALL of these are satisfied:

- [x] `docs/macro_spec.md` exists and documents each macro’s intent, required inputs (sheet names/columns), and output expectations (ranges/invariants).
- [x] Running the pytest suite (e.g., `pytest`) executes spec tests that cover the documented macro behaviors.
- [x] Spec tests fail with an assertion message identifying the deviating transformation when output differs from expected.
- [x] Spec tests include checks for MOSERS-format range-level outputs and invariants including “no blank numeric cells” and “headers match expected”.
- [x] `docs/macro_spec.md` includes a “known-acceptable drift” section with explicit tolerances/rounding rules.

### Recently Attempted Tasks
Avoid repeating these unless a task needs explicit follow-up:

- Create pytest spec tests using fixtures that perform range-level checks in generated MOSERS-format outputs.
- Add pytest spec tests that assert invariants like “no blank numeric cells” and “headers match expected”.
- Update repository to store VBA modules as source files in `assets/vba/*.bas` (if not already present).
