...
def _select_worksheet_and_headers(*, workbook: Any) -> tuple[Any, int, dict[str, int]]:
    best: tuple[Any, int, dict[str, int], tuple[str, ...], int] | None = None
    # Sort by score desc and then lowest header row for deterministic selection.
    for sheet_name in workbook.sheetnames:
        worksheet = workbook[sheet_name]
        row, header_columns, missing = _find_header_row_and_columns(worksheet=worksheet)
        score = len(_REQUIRED_HEADERS) - len(missing)
        if best is None or score > best[4] or (score == best[4] and row < best[1]):
            best = (worksheet, row, header_columns, missing, score)

    if best is None or best[4] <= 0:
        raise ValueError("Unable to locate worksheet containing expected NISA header text")
    ...
    counterparty_column = _infer_counterparty_column(
        worksheet=worksheet, header_columns=header_columns
    )
    if counterparty_column is None:
        raise ValueError(
            f"Missing required headers: counterparty (sheet '{worksheet.title}', header row {header_row})"
        )
    header_columns["counterparty"] = counterparty_column
