...
**Progress:** 9/11 tasks complete, 2 remaining
...
### Scope
Data quality problems such as unmapped names, missing segments, stale dates, skipped outputs, and limit breaches need one operator-facing report. A shared manifest object also lets automation decide whether a run is safe to use.

### Tasks
Complete these in order. Mark checkbox done ONLY after implementation is verified:

- [x] Define a `data_quality` manifest schema with severity levels `info`, `warn`, and `fail`, categorized findings, counts, and recommended actions.
- [x] Collect findings from reconciliation, mapping, date, limit, cash, and output-generation checks through `src/counter_risk/pipeline/data_quality.py`.
- [x] Write `DATA_QUALITY_SUMMARY.txt` with operator-friendly language and references to detailed files.
- [x] Update Runner UI to show quality status and link/open the summary file after a run.
- [x] Ensure manifest schema tests cover all-green, warning, and failure quality states.
- [x] Document how new checks should add findings to the shared report rather than inventing separate warning surfaces.

### Acceptance Criteria
The PR is complete when ALL of these are satisfied:

- [x] Every successful run writes a `data_quality` manifest object, even when there are no findings.
- [x] Runs with warnings or failures write `DATA_QUALITY_SUMMARY.txt` with actionable next steps and file references.
- [x] Runner UI displays quality status with no raw traceback language.
- [x] Tests verify manifest shape and UI/status behavior for green, warning, and failure cases.
- [x] New data-quality checks have a documented integration point and do not write unstructured warnings only.

### Recently Attempted Tasks
Avoid repeating these unless a task needs explicit follow-up:

- surface DATA_QUALITY_SUMMARY.txt status in the Tk runner result and UI
- Define a `data_quality` manifest schema with severity levels `info`, `warn`, and `fail`, categorized findings, counts, and recommended actions.

### Suggested Next Task
- Collect findings from reconciliation, mapping, date, limit, cash, and output-generation checks through `src/counter_risk/pipeline/data_quality.py`.
